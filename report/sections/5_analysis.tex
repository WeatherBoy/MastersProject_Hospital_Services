...
\\
We can define \(Q_a\) as the set of tasks which agent \(a\) isn't qualified to perform. Furthermore, we define \(D_a\) as the set of days where agent \(a\) is unable to perform any task. Finally, we can define \(R_j\) as the set of days where task \(j\) isn't performed. From these, we can already assign a bunch of our \(M \cdot N \cdot T\) decision variables, namely
\begin{align*}
