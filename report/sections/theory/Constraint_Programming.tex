...
Sometimes, the best way to understand an algorithm is just to see it in action. So, we will now run DPLL on \autoref{eq:clausal_form_example}. The first thing to note is that we can't use Unit Propagation on \autoref{eq:clausal_form_example}. This is largely due to it being a small example, and although we could artificially add the conjunction "\(\wedge p_4\)" to the original \autoref{eq:propositional_logic_example} without altering the satisfiability, we elected to keep it as is.
\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Constraint-Programming/DPLL-Toy-Example1.png}
        \subcaption{Initially, no literals has been assigned any values.}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Constraint-Programming/DPLL-Toy-Example2.png}
        \subcaption{We start with Pure Literal Elimination on \(p_5\) making the 4th clause true.}
    \end{subfigure}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \subcaption{Then we assign \texttt{False} to \(p_4\) at random.}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Constraint-Programming/DPLL-Toy-Example4.png}
        \subcaption{Next, when we try to assign \texttt{False} to \(p_2\), we get a conflict.}
    \end{subfigure}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \subcaption{We backtrack and try \texttt{True} for \(p_2\).}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Constraint-Programming/DPLL-Toy-Example6.png}
        \subcaption{Finally, when we assign \texttt{True} to \(p_1\), we have a satisfying assignment.}
    \end{subfigure}
\end{figure}
Note, that in this example \(p_3\) was never assigned a value. That is because for the assignment
\begin{equation*}
    \mathcal{A}: \{ p_1 = \text{\texttt{True}}, p_2 = \text{\texttt{True}}, p_4 = \text{\texttt{True}}, p_5 = \text{\texttt{True}} \},
\end{equation*} 
