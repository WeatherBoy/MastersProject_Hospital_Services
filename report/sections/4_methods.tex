...
\end{python}
Much like the above example, it is no more complex than a nested \texttt{for}-loop structure over tasks, days (in this case \texttt{days_off}) and tasks. By writing a simple \texttt{class} representation for our \texttt{agent} variables, we have easily readable information adhering to the specific agent, such as their unavailable days - stored in \texttt{days_off} as a \texttt{list} representation. Assigning each of these variables \(0\) is OR-Tools Python representation for setting them to false in the formula of our boolean satisfiability problem.
\\
\\ 
