...
\begin{verbatim}
    ...
    $
\end{verbatim}
Here, \verb|^| signifies the beginning of the string. \verb|([^\s(]+(?:_[^\s(]+)?)| captures \emph{group 1}. The first part, \verb|[^\s(]+| captures "one or more characters that are not whitespace \verb|\s| and are not the starting parenthesis character \verb|(|"\footnote{This is the standard formulation employed for regular expressions.}.